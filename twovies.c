...
char *getFileNameToRead()
{
    ...
    // Input Validation
    int ok = 0;
    // Loops until user enters a string that matches a .csv file in the directory.
    while (!ok)
    {
        // Gets user input
        scanf("%299[^\n]", lineEntered);
        // Creates a string to append .csv to whatever user entered.
        // This is done because maybe the user forgot the extension but entered a valid
        // file name.
        char csvExtAdded[304];
        sprintf(csvExtAdded, "%s.csv", lineEntered);

        FILE *file;
        // File exists, but may not have the right extension
        if (file = fopen(lineEntered, "r"))
        {
            fclose(file);
            char *type = ".csv";
            // Makes sure the valid file has .csv extension
            if (strcmp(type, &lineEntered[strlen(lineEntered) - 4]) == 0)
            {
                ok = 1;
            ...
            {
                printf("File has incorrect extension (must be .csv)\nEnter the name of the file: ");
            }
        }
        // Checks entered string + .csv extension
        else if (file = fopen(csvExtAdded, "r"))
        {
            fclose(file);
            ok = 1;
            nameToRead = csvExtAdded;
        }
